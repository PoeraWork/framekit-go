...
**编码器自动探测**：启动时依次尝试 `libx265 → hevc_amf → hevc_nvenc → hevc_qsv`，以第一个能实际打开的为准。GUI 中只展示当前机器可用的选项。

**B 帧缓冲**：HEVC 编码器内部维护 B 帧延迟缓冲，`SendFrame` 之后不一定立刻产出压缩包。`drain` 在每帧编码后都会调用，把当前已攒够的包全部写进 MP4；最后 `Close` 时发送 flush 信号再 drain 一次，确保尾帧不丢。
